...
import os
from probemanager.settings.base import ROOT_DIR, BASE_DIR
...
"""
    ...
    with open(ROOT_DIR + '/docs/modules/index.rst', 'w') as f:
        for app in apps.get_app_configs():
            for model in app.get_models():
                if issubclass(model, Probe):
                    if app.verbose_name != "Core":
                        path = BASE_DIR + "/" + app.label + "/README.rst"
                        if os.path.isfile(path):
                            ...
                            f_include = open(ROOT_DIR + '/docs/modules/' + app.label + '.rst', 'w')
