...
def git_tag(git_root):
    ...
    p = subprocess.Popen(command, shell=False, cwd=git_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                         universal_newlines=True)
    out, err = p.communicate()
    if err:  # pragma: no cover
        logger.exception(str(err))
    p_date = subprocess.Popen(command_date, shell=False, cwd=git_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                              universal_newlines=True)
