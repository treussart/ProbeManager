...
launch_celery(){
    if [ ! -f "$destfull"probemanager/celery.pid ]; then
        echo '## Start Celery ##'
        (cd "$destfull"probemanager/ && "$destfull"venv/bin/celery -A probemanager worker -D --pidfile celery.pid -B -l info --scheduler django_celery_beat.schedulers:DatabaseScheduler)
    else
        echo '## Restart Celery ##'
